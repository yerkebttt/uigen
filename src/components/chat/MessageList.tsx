...
function getToolDescription(
  ...
  const strip = (p: unknown) => String(p ?? "").replace(/^\//, "");

  if (toolName === "str_replace_editor") {
    const file = strip(args.path);
    switch (args.command) {
      case "create":     return done ? `Created ${file}`  : `Creating ${file}…`;
      case "str_replace":
      case "insert":     return done ? `Updated ${file}`  : `Updating ${file}…`;
      case "view":       return done ? `Read ${file}`     : `Reading ${file}…`;
      default:           return done ? `Edited ${file}`   : `Editing ${file}…`;
    }
  }

  if (toolName === "file_manager") {
    const file = strip(args.path);
    const newFile = strip(args.new_path);
    switch (args.command) {
      case "rename": return done ? `Renamed ${file} → ${newFile}` : `Renaming ${file}…`;
                      ...
                      {message.parts.map((part, partIndex) => {
                        switch (part.type) {
                          ...
                          case "tool-invocation": {
                            ...
                            const label = getToolDescription(
                              tool.toolName,
